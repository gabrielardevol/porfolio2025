...
@Component({
  ...
  template: `
      <div *ngIf="name == section || layout != 'vertical'  && !selectedProject"
           ...
           [ngClass]="layout == 'vertical' ? 'rounded-3xl gap-[1px]' : ''">
        ...
        <div *ngFor="let section of sections" class="bg-white" [ngClass]="section == name ? 'flex-1' : ''">
          ...
          <div *ngIf="(name == 'skills' || layout == 'vertical') && section == 'skills'"
            ...
            <p class="text-xl">
              El domini de software d'edició d'imatge, juntament amb la meva formació, em converteix en un perfil versàtil i polivalent.
            </p>
...
skill['rate'] - i >= 1 ? 'bi-star-fill' : skill['rate'] - i > 0 && skill['rate'] - i < 1 ? 'bi-star-half' : 'bi-star'">
          ...
          <div *ngIf="(name == 'contact' || layout == 'vertical') && section == 'contact' && !selectedProject "
               ...
               [ngClass]="layout == 'vertical' ? 'rounded-3xl' : ''">
            <img src="https://t3.ftcdn.net/jpg/05/16/27/58/360_F_516275801_f3Fsp17x6HQK0xQgDQEELoTuERO4SsWV.jpg" alt="" class="max-w-[300px] aspect-square rounded-xl">
            <br>
            <div class="flex gap-2"><i class="bi bi-whatsapp"></i> <span class="font-medium">+34 646 18 16 10</span></div>
            <div class="flex gap-2"><i class="bi bi-envelope-fill"></i> <span class="font-medium">artsdevol&#64;gmail.com</span></div>
            <div class="flex gap-2"><i class="bi bi-linkedin"></i> <a class="underline font-medium" target="_blank" href="https://www.linkedin.com/in/gabrielardevol/">Linkedin</a></div>
            <div class="flex gap-2"><i class="bi bi-github"></i> <a class="underline font-medium" target="_blank" href="https://github.com/gabrielardevol">Github</a></div>
          </div>
...
<!--                        [ngClass]="layout == 'vertical' ? 'rounded-3xl' : ''"/>-->
          ...
          <div *ngIf="(layout == 'vertical' || layout == 'detail' || name == 'projects') && section == 'projects'" class="w-full grid transition-all duration-500 overflow-hidden bg-white rounded-3xl"
            ...
            >
              <div class="px-5 bg-white" [ngClass]="selectedProject ? 'hidden' : '' ">
                ...
                <p class="text-xl bg-white">
                  El domini de software d'edició d'imatge, juntament amb la meva formació, em converteix en un perfil versàtil i
                  polivalent.
                </p>
              </div>
              <app-project
                *ngFor="let project of [1, 2, 3, 4, 5], let i = index"
                [ngClass]="selectedProject && selectedProject!== i ? 'hidden' : '' "
                (click)="layoutService.setLayout('detail'); layoutService.selectProject(i)"
                [index]="i"/>
            </div>
...
void {
  ...
  this.subscription.add(
    this.layoutService.syncSection$().subscribe((valor: SectionType) => {
      this.section = valor;
    }),
  );
  this.subscription.add(
    this.layoutService.syncProject$().subscribe((valor: number | undefined) => {
      this.selectedProject = valor;
    }),
  )
}
