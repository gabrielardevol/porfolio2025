...
@Component({
  ...
  standalone: true,
  imports: [RouterOutlet, NgClass, NgForOf, NgIf, SquiggleComponent, ProjectDetailComponent, NgOptimizedImage],
  styles: [
    `
      .section {
        @apply rounded-3xl;
  ...
  template: `
    <div *ngIf="clickedSection == section || layout != 'vertical'  && !selectedProject"
      ...
      <div *ngFor="let section of sections" class="bg-white" [ngClass]="section == clickedSection ? 'flex-1' : ''">

        <!--      SKILLS-->
        <div *ngIf="(clickedSection == 'skills' || layout == 'vertical') && section == 'skills'"
             class="p-4 flex flex-col gap-3 bg-white section"
             [ngClass]="layout == 'vertical' ? 'rounded-3xl' : ''">
          <p class="text-xl">
            El domini de software d'edició d'imatge, juntament amb la meva formació, em converteix en un
            perfil versàtil
            i polivalent.
          </p>
...
skill['rate'] - i >= 1 ? 'bi-star-fill' : skill['rate'] - i > 0 && skill['rate'] - i < 1 ? 'bi-star-half' : 'bi-star'">
        ...
        <!--      ABOUT-->
        <div *ngIf="(clickedSection == 'about' || layout == 'vertical') && section == 'about' && !selectedProject"
        ...
        <!--      CONTACT-->
        <div *ngIf="(clickedSection == 'contact' || layout == 'vertical') && section == 'contact' && !selectedProject "
             class="p-4 flex flex-col gap-1 bg-white section h-full items-center justify-center text-xl"
             [ngClass]="layout == 'vertical' ? 'rounded-3xl' : ''">
<!--          <img [ngSrc]="'../assets/avatar.png'"-->
<!--               alt=""-->
<!--               class="max-w-[300px] aspect-square rounded-xl" height="454" width="430">-->
          <br>
          <div class="flex gap-2"><i class="bi bi-whatsapp"></i> <span
            class="font-medium">+34 646 18 16 10</span></div>
          <div class="flex gap-2"><i class="bi bi-envelope-fill"></i> <span
            class="font-medium">artsdevol&#64;gmail.com</span></div>
          <div class="flex gap-2"><i class="bi bi-linkedin"></i> <a class="underline font-medium"
                                                                    target="_blank"
          ...
          <button class="rounded-full border-2 border-black px-7 py-1 hover:bg-black hover:text-white font-black">
            CV
          </button>
        </div>
        ...
        <!--      PROJECTS-->
        <div
          *ngIf="(layout == 'vertical' || layout == 'detail' || clickedSection == 'projects') && section == 'projects'"
          ...
          >
            <div class="px-5 bg-white" [ngClass]="selectedProject ? 'hidden' : '' ">
              ...
              <p class="text-xl bg-white">
                El domini de software d'edició d'imatge, juntament amb la meva formació, em converteix en
                un perfil
                versàtil i
                polivalent.
              </p>
